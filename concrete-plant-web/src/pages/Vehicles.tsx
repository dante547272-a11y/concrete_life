/**
 * Vehicles Management Page with Map Overview
 */
...
// Mock vehicles with locations around Hangzhou
...
const Vehicles: React.FC = () => {
  ...
  const filteredVehicles = mockVehicles.filter(v => 
    ...
    (v.driver && v.driver.includes(searchText))
  );
  ...
  // Vehicle list panel for map view
    ...
    <div style={{ 
      ...
      <div style={{ padding: 12, borderBottom: '1px solid var(--border-color)' }}>
        ...
        <div style={{ marginLeft: 20, marginTop: 4 }}>
          <Checkbox checked>默认分组</Checkbox>
        </div>
      </div>
      ...
      <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--border-color)', display: 'flex', gap: 8 }}>
        <Tag color="blue">全部({mockVehicles.length})</Tag>
        <Tag color="green">在线({vehiclesWithLocation.length})</Tag>
        <Tag>离线({mockVehicles.length - vehiclesWithLocation.length})</Tag>
      </div>
        ...
        <table style={{ width: '100%', fontSize: 12 }}>
          <thead>
            <tr style={{ background: 'var(--bg-tertiary)' }}>
              ...
              <th style={{ padding: '8px 4px', textAlign: 'left', color: 'var(--text-secondary)' }}>车牌</th>
              <th style={{ padding: '8px 4px', textAlign: 'left', color: 'var(--text-secondary)' }}>当前状态</th>
            </tr>
          </thead>
          <tbody>
            {filteredVehicles.map(v => (
              <tr key={v.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={{ padding: '6px 4px', color: 'var(--text-primary)' }}>{v.plateNumber}</td>
                <td style={{ padding: '6px 4px' }}>
                  {v.status === 'in_use' && v.eta ? (
                    <Tag color="green" style={{ fontSize: 10 }}>距离{v.eta}</Tag>
                  ) : v.status === 'available' ? (
                    <Tag color="cyan" style={{ fontSize: 10 }}>静止中</Tag>
                  ) : v.status === 'maintenance' ? (
                    <Tag color="orange" style={{ fontSize: 10 }}>维护中</Tag>
                  ) : (
      ...
      <div style={{ flex: 1, position: 'relative' }}>
            ...
            .map(vehicle => (
              ...
              >
                <Tooltip permanent direction="top" offset={[0, -20]}>
                  <div style={{ fontSize: 11, fontWeight: 600 }}>{vehicle.plateNumber}</div>
                </Tooltip>
                <Popup>
                  <div style={{ minWidth: 180 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 8, borderBottom: '1px solid #eee', paddingBottom: 8 }}>
                      {vehicle.plateNumber}
                    </div>
                    <div style={{ fontSize: 12, lineHeight: 1.8 }}>
                      <div><strong>司机:</strong> {vehicle.driver}</div>
                      <div><strong>类型:</strong> {typeLabels[vehicle.vehicleType]}</div>
                      <div><strong>状态:</strong> <Tag color={statusColors[vehicle.status]} style={{ marginLeft: 4 }}>{statusLabels[vehicle.status]}</Tag></div>
                      {vehicle.speed !== undefined && <div><strong>速度:</strong> {vehicle.speed} km/h</div>}
                      {vehicle.destination && <div><strong>目的地:</strong> {vehicle.destination}</div>}
                      {vehicle.eta && <div><strong>预计到达:</strong> {vehicle.eta}</div>}
                    </div>
                  </div>
                </Popup>
              </Marker>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            <div style={{ width: 16, height: 16, background: '#1890ff', borderRadius: '50%' }}></div>
            <span style={{ color: '#333' }}>运输中</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            <div style={{ width: 16, height: 16, background: '#52c41a', borderRadius: '50%' }}></div>
            <span style={{ color: '#333' }}>空闲</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 16, height: 16, background: '#faad14', borderRadius: '50%' }}></div>
            <span style={{ color: '#333' }}>维护中</span>
          </div>
        </div>
      </div>
    </div>
